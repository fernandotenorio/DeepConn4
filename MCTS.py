...
class MCTS():
    ...
    def search(self, game):
        ...
        if s not in self.Ps:
            # leaf node            
            p, v = self.nnet.predict(np.array([game.encode_board()]))
            self.Ps[s] = p[0]
            v = v[0][0]
            valids = game.get_actions()
            valids = np.array([1 if i in valids else 0 for i in range(game.action_size())])            
